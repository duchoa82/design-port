...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: process.env.ADMIN_EMAIL || 'duchoa201093@gmail.com',
...
const templates = {
  tokenRequest: (data) => ({
    from: process.env.ADMIN_EMAIL || 'duchoa201093@gmail.com',
    to: process.env.ADMIN_EMAIL || 'duchoa201093@gmail.com',
...
export async function sendTokenRequestNotification(data) {
  try {
    ...
    console.log('✅ Token request notification sent:', result.messageId);
...
export async function sendTokenApprovalNotification(data) {
  try {
    ...
    const result = await transporter.sendMail(template);
    console.log('✅ Token approval notification sent:', result.messageId);
...
export async function testEmailConfiguration() {
  try {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: #e3f2fd; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <h2 style="color: #1976d2; margin: 0;">Gmail SMTP Test Successful!</h2>
          </div>
          
          <div style="background: white; padding: 20px; border: 1px solid #dee2e6; border-radius: 8px;">
            <p style="color: #495057; margin: 0;">
              This is a test email from your Token Management System using Gmail SMTP. 
              If you receive this, your email configuration is working correctly!
            </p>
            
            <div style="margin-top: 20px; padding: 15px; background: #d4edda; border-radius: 5px;">
              <p style="margin: 0; color: #155724;">
                <strong>✅ Gmail SMTP Status:</strong> Ready for production
              </p>
            </div>
          </div>
    ...
    console.error('❌ Gmail SMTP configuration is invalid:', error);
    return false;
  }
}
